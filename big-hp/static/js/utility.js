...
function resizeFrameBodyMinusButtonsHeight(frameId,fallbackHeightValue) {
    try {
        var viewportHeight = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
        var bannerHeight =  document.getElementById("banner").clientHeight;
        var headerHeight =  document.getElementById("header").clientHeight;
        var alertHeight = document.getElementById("alert").clientHeight;
        var scrollTop = $(window).scrollTop();
        var newTop = Math.max(0, (bannerHeight + headerHeight + alertHeight) - scrollTop);

        document.getElementById("helppanel").style.top = newTop + 'px';

        // -50px to account for the buttons and padding above the help iframe
        var newHeight = viewportHeight - newTop - 50;
    ...
    // Non accessible content (different domain)
        ...
        if (fallbackHeightValue) {
            document.getElementById(frameId).style.height = fallbackHeightValue + 'px';
        }
    }
}
